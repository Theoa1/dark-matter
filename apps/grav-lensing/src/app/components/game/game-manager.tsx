...
import { DarkMatter, getLensEffectData, getTransformMatrixForDistortion } from "../../util/distort"
...
export const GameManager = ({
  ...
  const [darkMatterMassRange, setDarkMatterMassRange] = useState<[number, number]>(defaultDarkMatterMassRange)
  const gameGrid = useRef<GridSquareScanState[]>([])
  const gameOver = useRef<boolean>(false)

  const scanGridSquare = useCallback((pos: [number, number]): ScanResult => {
    ...
    scanResult.distortion = getTransformMatrixForDistortion(getLensEffectData(darkMatter, pos))
    gameGrid.current[gridIndex].scanResult = scanResult
    setFunding((prevValue) => {
  ...
  const revealGridSquare = useCallback((pos: [number, number]): ScanResult | undefined => {
    if (!gameOver.current) {
  ...
  const getEncodedUrl = useCallback(() => {
    if (darkMatter.length > 0) {
      ...
      if (window.location.ancestorOrigins && window.location.ancestorOrigins.length > 0) {
        console.log(`Ancestor origins, probably framed. Getting top level url: ${window.location.ancestorOrigins[window.location.ancestorOrigins.length - 1]}`)
        const url = new URL(window.location.ancestorOrigins[window.location.ancestorOrigins.length - 1])
        url.searchParams.set('game', packet)
        return url.toString()
      } else {
        console.log(`Creating base url from route: ${Routes.Game}/custom and host: ${window.location.origin}`)
  ...
  useEffect(() => {
    if (darkMatterOverride) {
      ...
      setDarkMatter(getRandomisedDarkMatter(gridSize, darkMatterCount, darkMatterMassRange))
    }
