...
import { ScanResult, useGameDarkMatterRemaining, useGameFunding, useGameScanGridSquare, useGameRevealGridSquare} from './game-manager-context'
...
export const GameView = ({cols, rows, difficulty}:GameViewProps) => {
  ...
  useEffect(() => {
    if (showEndScreen) {
      amplitude.getInstance().logEvent('Player completed game', {funding, difficulty})
    }
  }, [showEndScreen, funding, difficulty])

  const handleScanGridClick = (position: [number, number], existingResult?: ScanResult) => {
    setFirstClick(false)
    ClickSound.play()
  ...
  const handleRevealGrid = useCallback((position: [number, number])=>{
  ...
  return(
    <GameViewContainer>
      ...
      <GameGridContainer>
        <Grid columns = {cols} rows = {rows} scanGridCallback={handleScanGridClick} revealGridCallback = {handleRevealGrid} displayAll = {showEndScreen}/>
      </GameGridContainer>
